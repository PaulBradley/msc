...
miss-spellings returned as `JSON`, along with a list of suggested replacements.

Users of the API will be able to request for new words to be added to the
...
For example, the calling system could pass in the patients name, so that it is
not triggered as a miss-spelling.